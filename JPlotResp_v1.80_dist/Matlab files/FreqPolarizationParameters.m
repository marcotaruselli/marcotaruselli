...
% addpath(genpath('C:\UNIMORE\Seismic noise\20180129_Esperimento Lierna\'));
%% Open files -----------------------------------------------------------
...
timeWindow = 10;                                                            % Time window lenght (for averaging) [s]
...
for Ifilt = 1:length(bandPassFilters)
    ...
    %
    betaSquaredSVDTrace = zeros(1,fftSample/2);
    betaSquaredEIG = zeros(1,fftSample/2);
    thetaH = zeros(1,fftSample/2);
    thetaV = zeros(1,fftSample/2);
    phiHH = zeros(1,fftSample/2);
    phiVH = zeros(1,fftSample/2);
    %
    timeWindowsTapeMatrix = repmat(timeWindowSTape,1,winNumber);        % Tapering
    for K = 1:winNumber
        %             dataTemp = tempData(((K-1)*timeWindowS)+1:(((K-1)+1)*timeWindowS),:).*timeWindowSTape3C;
        dataTemp = data(winIndexes(K):winIndexes(K)+timeWindowS-1,:).*timeWindowSTape3C;
        eventFreqTemp = fft(dataTemp,fftSample,1);
        %             eventFreqTemp = eventFreqTemp(1:fftSample/2,:);
        %             eventFreqTemp(:,1) = sum(repmat(eventFreqTemp(:,1),1,fftSample/2).*konnoOhmachi,1,'omitnan');
        %             eventFreqTemp(:,2) = sum(repmat(eventFreqTemp(:,2),1,fftSample/2).*konnoOhmachi,1,'omitnan');
        %             eventFreqTemp(:,3) = sum(repmat(eventFreqTemp(:,3),1,fftSample/2).*konnoOhmachi,1,'omitnan');
        autoSpectX(:,K) = eventFreqTemp(1:fftSample/2,1).*conj(eventFreqTemp(1:fftSample/2,1));
        autoSpectY(:,K) = eventFreqTemp(1:fftSample/2,2).*conj(eventFreqTemp(1:fftSample/2,2));
        autoSpectZ(:,K) = eventFreqTemp(1:fftSample/2,3).*conj(eventFreqTemp(1:fftSample/2,3));
        crossSpectXY(:,K) = eventFreqTemp(1:fftSample/2,1).*conj(eventFreqTemp(1:fftSample/2,2));
        crossSpectXZ(:,K) = eventFreqTemp(1:fftSample/2,1).*conj(eventFreqTemp(1:fftSample/2,3));
        crossSpectYZ(:,K) = eventFreqTemp(1:fftSample/2,2).*conj(eventFreqTemp(1:fftSample/2,3));
    end
    ...
    for I = freqMinSample:fftSample/2
        ...
        betaSquaredSVDTrace(I) = (3*trace(S.^2) - (trace(S))^2)/(2*((trace(S))^2));
        % betaSquaredEIG(I) = (2*(eigValue(1,1)-eigValue(2,2))^2 + 2*(eigValue(1,1)-eigValue(3,3))^2 + 2*(eigValue(2,2)-eigValue(3,3))^2)/(4*(trace(eigValue))^2);
        % betaSquaredEIGTrace(I) = (3*trace(eigValue.^2) - (trace(eigValue))^2)/(2*((trace(eigValue))^2));
        % POLARIZATION PARAMETERS - Polarization angles -------------------
        % Backazimuth angle
        tempFunct2 = abs(real((sqrt(V(1,1)^2+V(2,1)^2)).*exp(1i*2*pi*freqAx(I)*(0:(1/freqAx(I))/500:1/freqAx(I)-(1/freqAx(I))/500))));
        ...
        thetaH(I) = mod(atan2d(real(V(2,1)*exp(-1i*thetaHtemp2)),real(V(1,1)*exp(-1i*thetaHtemp2))),360);     % Range 0°/+360°
        % Incident angle
        thetaVtemp = -0.5*angle(V(3,1)^2+V(1,1)^2+V(2,1)^2) + pi/2;
        thetaV(I) = atand(abs((real(V(3,1)*exp(-1i*thetaVtemp)))/(real((sqrt(V(1,1)^2+V(2,1)^2))*exp(-1i*thetaVtemp)))));
        % Phase lag between principal horizontal components
        phiHH(I) = rad2deg(wrapToPi((angle(V(1,1)))-(angle(V(2,1)))));
        % Phase lag between principal horizontal and vertical components
        phiVH(I) = rad2deg(thetaHtemp2-angle(V(3,1)));
    end
    ...
    for I = 1:size(fileNamesBlock,1)
        %     [coeff,score,latent,tsquared,explained,mu] = pca(dataBlockNoResp{1});
        [coeff(:,:,I),score{I},latent(:,I),~,explained(:,I),~] = pca(data);
        %     % Rotation of the PCA eigenvectors ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        %     rotatedCoeffV(:,:,I) = rotatefactors(coeff(:,1:2,I));
        %     rotatedCoeffP(:,:,I) = rotatefactors(coeff(:,1:3,I),'Method','promax');
    end
    ...
    for I = 1:size(fileNamesBlock,1)
        ...
        hold on
        hleg(1) = plot([-coeff(1,1,I) coeff(1,1,I)],[-coeff(2,1,I) coeff(2,1,I)],'-r','linewidth',2);
        hleg(2) = plot([-coeff(1,2,I) coeff(1,2,I)],[-coeff(2,2,I) coeff(2,2,I)],'--g','linewidth',2);
        hleg(3) = plot([-coeff(1,3,I) coeff(1,3,I)],[-coeff(2,3,I) coeff(2,3,I)],':b','linewidth',2);
        ...
        hold on
        plot([-coeff(1,1,I) coeff(1,1,I)],[-coeff(3,1,I) coeff(3,1,I)],'-r','linewidth',2);
        plot([-coeff(1,2,I) coeff(1,2,I)],[-coeff(3,2,I) coeff(3,2,I)],'--g','linewidth',2);
        plot([-coeff(1,3,I) coeff(1,3,I)],[-coeff(3,3,I) coeff(3,3,I)],':b','linewidth',2);
        %     % Rotated eigenvectors
        ...
        hold on
        plot([-coeff(2,1,I) coeff(2,1,I)],[-coeff(3,1,I) coeff(3,1,I)],'-r','linewidth',2);
        plot([-coeff(2,2,I) coeff(2,2,I)],[-coeff(3,2,I) coeff(3,2,I)],'--g','linewidth',2);
        plot([-coeff(2,3,I) coeff(2,3,I)],[-coeff(3,3,I) coeff(3,3,I)],':b','linewidth',2);
        %     % Rotated eigenvectors
